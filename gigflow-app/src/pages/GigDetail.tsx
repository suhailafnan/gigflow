  ...
  return (
    <div className="space-y-6">
      <div className="pixel-card p-6">
        <h2 className="text-3xl font-bold text-red-500 mb-2">{gig.title}</h2>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          <div className="pixel-card p-6">
            <h3 className="font-bold text-red-500 mb-3">Milestones</h3>
            <ul className="space-y-2">
              {gig.milestones.map((ms, i) => (
                <li key={i} className="flex justify-between items-center p-2 border border-red-900 rounded">
        ...
        <div className="space-y-6">
          <div className="pixel-card p-6">
            <h3 className="font-bold text-red-500 mb-3">Actions</h3>
                  ...
                  <ul className="space-y-1">
                    {gig.applicants.map(addr => (
                      <li key={addr} className="flex justify-between items-center text-sm">
                        ...
                        <button onClick={() => handleAccept(addr)} className="text-xs px-2 py-1 border border-red-500 rounded hover:bg-red-500">Accept</button>
                      </li>
                    ))}
                  </ul>
            ...
            {!isOwner && gig.status === "Open" && (
              <button onClick={handleApply} disabled={hasApplied} className="w-full px-4 py-2 border border-red-500 rounded disabled:opacity-50">
